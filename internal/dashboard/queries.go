...
// FetchTimeSeriesData returns date+disposition+count grouped data for Chart.js.
func FetchTimeSeriesData(db *gorm.DB, startDate, endDate time.Time, domains, orgs []string) (*ChartData, error) {
	endDateInclusive := endDate.AddDate(0, 0, 1)
	...
	var rows []dayRow
	err := db.Table("records").
		Select("DATE(reports.begin_date) as date, records.disposition, SUM(records.count) as total").
		Joins("JOIN reports ON reports.id = records.report_id").
		Where("reports.domain IN ?", domains).
		Where("reports.org_name IN ?", orgs).
		Where("reports.begin_date >= ? AND reports.begin_date <= ?", startDate, endDateInclusive).
		Group("date, records.disposition").
		Order("date").
		Scan(&rows).Error
	...
	// Pivot: collect unique dates and dispositions, build matrix
	dateSet := make(map[string]bool)
	dispSet := make(map[string]bool)
	dataMap := make(map[string]map[string]int) // date -> disposition -> count
	...
	for _, r := range rows {
		...
		if dataMap[r.Date] == nil {
			dataMap[r.Date] = make(map[string]int)
		}
		dataMap[r.Date][disp] = r.Total
	}
	...
	// Build datasets (fillna(0) equivalent)
	var datasets []ChartDataset
	for _, disp := range dispositions {
		var data []int
		for _, date := range dates {
			data = append(data, dataMap[date][disp])
		}
